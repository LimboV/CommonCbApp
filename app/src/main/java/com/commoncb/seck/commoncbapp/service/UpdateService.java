package com.commoncb.seck.commoncbapp.service;
...
import com.commoncb.seck.commoncbapp.utils.MemoryStatus;
...
public class UpdateService extends Service {
    ...
    /*String[] command = {"chmod", "777", updateFile.getPath()};
               ...
               /* try {
                    String command = "chmod 777 " + updateFile.getAbsolutePath();
                    Log.i("limbo", "command = " + command);
    ...
    private void showChannel1Notification() {
        updateNotificationManager = (NotificationManager) UpdateService.this.getSystemService(Context.NOTIFICATION_SERVICE);
        updateNotification = new Notification();
        ...
        Notification.Builder builder = new Notification.Builder(UpdateService.this, "1");
        ...
        updateNotificationManager.notify(notificationId, updateNotification);
        //设置下载进度条
        ...
        //延迟意图
        updateIntent = new Intent(this, MainActivity.class);
        updatePendingIntent = PendingIntent.getActivity(this, R.string.app_name, updateIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        updateNotification.contentIntent = updatePendingIntent;
        updateNotification.flags |= Notification.FLAG_ONGOING_EVENT;// 滑动或者clear都不会清空
        updateNotification.flags = Notification.FLAG_ONLY_ALERT_ONCE;
        ...
        protected void onPostExecute(Integer result) {
            ...
            if (result == DOWNLOAD_COMPLETE) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    // 7.0+以上版本
                    Uri apkUri = FileProvider.getUriForFile(UpdateService.this, "com.commoncb.seck.commoncbapp.myprovider", updateFile); //与manifest中定义的provider中的authorities="com.xxxx.fileprovider"保持一致
                ...
                updateNotification.contentIntent = updatePendingIntent;
                updateNotification.contentView.setTextViewText(
                        R.id.download_notice_speed_tv, "下载結束");
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    //关闭通知通道
                    updateNotificationManager.deleteNotificationChannel("1");
                }
                //启动安装程序
                ...
                updateNotification.when = System.currentTimeMillis();
                updateNotification.contentView.setTextViewText(
                        R.id.download_notice_speed_tv, "下载失败");
                updateNotification.flags |= Notification.FLAG_AUTO_CANCEL;
                updateNotification.defaults = Notification.DEFAULT_SOUND;
                updateNotificationManager.notify(0, updateNotification);
                stopSelf();
                            ...
                            if (is != null) {
                                ...
                                while ((readsize = is.read(buffer)) > 0) {
                                    ...
                                    if ((count == 0)
                                        ...
                                        count += 5;
                                        updateNotification.contentView.setTextViewText(R.id.download_notice_speed_tv,
